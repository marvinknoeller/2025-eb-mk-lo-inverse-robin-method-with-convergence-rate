...
# ghandleref = inner(n,grad(u_ex)) + alpha_exact * u_ex
# -----------------------------------------------------------------------------#

# fhandle = lambda x,y : -1 *(x+1) * ufl.sin(4*np.pi*y)
...
# u_ex1 =  x[0]**2 * ufl.cos(16*np.pi*x[1]) + 0.1
# qq = fem.Function(V)
...
# cos_coeffs1 = fem.Constant(domain, default_scalar_type(np.array([10.0, 1.0, -.5, 2.0, 1.0, -.5])))
# sin_coeffs1 = fem.Constant(domain, default_scalar_type(np.array([.2, 1.0, -.5, 2.0, 1.0, -.5])))
# alpha_exact1 = create_fun(cos_coeffs1, sin_coeffs1, x)
# ghandle = inner(n,grad(u_ex1)) + alpha_exact1 * u_ex1
# -----------------------------------------------------------------------------#
...
savemat('reconstructionV1backtrack_'+str(num_cos)+'_'+str(num_sin)+'delta0pz'+'.mat',data)